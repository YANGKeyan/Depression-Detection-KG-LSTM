...
# 定义了一个用于限制梯度裁剪的函数clipgradient，接受两个参数：模型model和裁剪值clipvalue。
# 通过使用filter()函数，过滤掉梯度为None的参数，并在对应参数的梯度上进行裁剪，以使其在[-clipvalue, clipvalue]的范围内。
# 这样可以避免梯度爆炸或梯度消失的问题，从而有效地提高训练模型的稳定性。
...
def train_model(model, train_iter,dev_iter , epoch, lr, loss_func):
    # 使用过滤函数 filter 从 model 的参数中过滤出需要计算梯度的参数
    ...
    for idx, batch in enumerate(train_iter):
        ...
        target = batch.label
        #对训练数据进行迭代，若GPU可用则将输入数据移至GPU上，调用模型并计算损失。
        ...
        if idx % 30 == 0:
            logger.info('Epoch:%d, Idx:%d, Training Loss:%.4f', epoch, idx, loss.detach().item())
