...
#- dev_data_path : 验证/开发数据集的路径。如果为 None，则表示没有验证/开发集。
#- test_data_path : 测试数据集的路径。如果为 None，则表示没有测试集。
#- txt_wordVectors_path : 用于文本序列词汇表的预先训练的词向量文件路径。如果为 None，则表示不使用预训练的词向量。
#- cpt_wordVectors_path : 用于概念序列词汇表的预先训练的词向量文件路径。如果为 None，则表示不使用预训练的词向量。
#- train_batch_size : 训练集的批次大小。
#- dev_batch_size : 验证/开发集的批次大小。如果为 None，则表示不使用验证/开发集。
#- test_batch_size : 测试集的批次大小。如果为 None，则表示不使用测试集。
